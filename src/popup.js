...
var br = isChrome ? chrome : browser;
...
var _Templates = __webpack_require__(330);

var _Ashe = __webpack_require__(331);

var DOMAIN_REGEX = /(?:[\w-]+\.)*([\w-]{1,63})(?:\.(?:\w{3}|\w{2}))(?:$|\/)/i; /**
...
var render = function render() {
    _Browser.br.tabs.getSelected(null, function (tab) {
        _Browser.br.runtime.sendMessage({ action: 'show_vulnerabilities', tab_id: tab.id }, doRender);
    });
};

var doRender = function doRender(response) {
    ...
    if (isCheckedAll()) {
        return doRenderContent(_Templates.TEMPLATES.DATA, { domains: Object.keys(data).filter(extractDomain).sort().map(function (d) {
                return {
                    name: d,
    ...
    if (!data[host]) {
        if (!Object.keys(data).length) {
            return doRenderContent(_Templates.TEMPLATES.HELP);
        }

        return extractDomain(url) ? doRenderContent(_Templates.TEMPLATES.NONE, {}) : doRenderContent(_Templates.TEMPLATES.WRONG, { url: url });
    }
    ...
    doRenderContent(_Templates.TEMPLATES.DATA, { domains: domains });
};

/**
 * Set innerHTML of content element
 */
var doRenderContent = function doRenderContent(template, data) {
    document.getElementById('content').innerHTML = (0, _Ashe.parse)(template, data);
    ...
    document.querySelectorAll('a').forEach(function (a) {
        return a.addEventListener('click', function (e) {
            return openLink(e.target.href || e.target.parentElement.href);
        });
    });
};

var doRenderHeader = function doRenderHeader(stat) {
...
var doRenderSettings = function doRenderSettings() {

    doRenderContent(_Templates.TEMPLATES.SETTINGS);
    document.querySelector('.material-icons.delete').addEventListener('click', onClearClick);
};

var doRenderIndex = function doRenderIndex() {
    var startBtn = document.getElementById('start');

    if (!startBtn) return;
    startBtn.addEventListener('click', function () {
        return _Browser.br.runtime.sendMessage({ action: 'start' }, function (resp) {
...
var onClearClick = function onClearClick() {
    _Browser.br.tabs.getSelected(null, function (tab) {
        return _Browser.br.runtime.sendMessage({ action: 'clear_data', tab_id: tab.id }, doRender);
    });
};

var sortSorftByScore = function sortSorftByScore(soft) {
    return Object.keys(soft).sort(function (a, b) {
        return soft[b].score - soft[a].score;
    }).map(function (s) {
        return soft[s];
    });
};
...
var openLink = function openLink(url) {
    return url && _Browser.br.runtime.sendMessage({ action: 'open_link', url: url });
};
...
var isCheckedAll = function isCheckedAll() {
    return document.getElementById('show_all_content').checked;
};
...
_Browser.br.runtime.sendMessage({ action: 'get_templates' }, function (response) {
 ...
 */
process = function process(str, data) {
        ...
        if (!token.expr) {
            ...
            switch (token.expr.type) {
                case 'if':
                    ...
                    block = token.buffer.match(cond ? /\{%\s*if\s+.+?\s*%\}([\s\S]*?)\{%/i : /\{%\s*else\s*%\}([\s\S]*?)\{%/i);
 ...
 */
proccessMarkers = function proccessMarkers(str) {
    var i = 0;
    str = trim(str);
 ...
 */
proccessControls = function proccessControls(str, i, lookingFor, exprDescr, inline) {
    var from = i;
        ...
        if (inline || lookingFor && expr.match(lookingFor)) {
            ...
            // If statement
                ...
                // Set expression
                        ...
                        str = m[2] ? proccessControls(str, i, null, dat, true) : proccessControls(str, i + 2, /\s*endset\s*/i, dat);
                    }
        }
        i = i + repl.length;
    }
 ...
 * @param {Array|Object}
 */
hasElements = function hasElements(obj) {
 ...
 */
replaceWith = function replaceWith(str, replace, start, end) {
 ...
 */
__parse = function __parse(tplStr, tplData) {
 ...
 * Add new modifiers.
 */
__addModifiers = function __addModifiers(obj) {
    for (var i in obj) {
...
exports.modifiers = __modifiers;
exports.addModifiers = __addModifiers;
